...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            ...
            //目标位置
            this.targetExport=this.trigger.querySelector(params.targetExport);
            if(params.valueTo){
        ...
        finish: function(e) {
            ...
            //触发元素为文本框时，将结果输入至value
            _self.trigger.value = provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''));
            //触发元素为一般元素，将结果输入至自定义属性data-value
            _self.trigger.setAttribute("data-value",provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):('')));
            _self.trigger.setAttribute("data-province",provinceText);
            _self.trigger.setAttribute("data-city",cityText);
            _self.trigger.setAttribute("data-county",countyText||null);
            //结果输出至目标位置
            _self.targetExport.textContent=provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''));
            _self.value = [provinceVal, cityVal, countyVal];
            _self.targetExport.setAttribute("val",_self.value);
            if(this.valueTo){
